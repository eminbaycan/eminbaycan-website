...
export function Caught({ lang }: CaughtProps) {
  ...
  useEffect(() => {
    // DevTools inceleme engelleme tuzağı (Debugger Trap)
    ...
    // Dinamik Sayfa Yüklenme Hızı Ölçümü (Performance API)
    let calculatedLoadSpeed = 0.24;
    try {
      const [navEntry] = performance.getEntriesByType('navigation') as PerformanceNavigationTiming[];
      if (navEntry && navEntry.duration > 0) {
        calculatedLoadSpeed = Number((navEntry.duration / 1000).toFixed(2));
      } else if (performance.timing && performance.timing.loadEventEnd > performance.timing.navigationStart) {
        const duration = performance.timing.loadEventEnd - performance.timing.navigationStart;
        if (duration > 0) {
          calculatedLoadSpeed = Number((duration / 1000).toFixed(2));
        }
      } else {
        calculatedLoadSpeed = Number((performance.now() / 1000).toFixed(2));
      }
    } catch {
      calculatedLoadSpeed = 0.24;
    }

    if (calculatedLoadSpeed <= 0 || isNaN(calculatedLoadSpeed)) {
      calculatedLoadSpeed = 0.15;
    }
    ...
    const targetStats = { 
      ...
      loadSpeed: calculatedLoadSpeed, // Dinamik ölçülen saniye
      uptime: 99.99,
      perfScore: 98
    };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[70vh] text-center px-4 animate-in fade-in zoom-in duration-500 py-12">
      ...
      <p className="mt-8 text-sm opacity-60 font-medium max-w-lg italic">
        {lang === 'tr' 
